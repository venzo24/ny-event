...
module.exports = class PingCommand extends Command {
  constructor(context, options) {
    super(context, { ...options, description: 'give  cookies', preconditions: ['headModOnly'] });
  }
  ...
  async chatInputRun(ctx) {
    ...
    if (!target) {
      ...
      await ctx.reply({ content: 'Вы не можете дать себе печенье!', ephemeral: true });
      return;
    }
    ...
    if (!recipient) {
      ...
      return;
    } else if (amount > user.cookies) {
      await ctx.reply({ content: 'Вы не можете дать больше печенья чем у вас есть!', ephemeral: true });
      return;
    }
    ...
    await ctx.reply(`Вы успешно передали ${amount} печенек пользователю ${target.username}`);
  }
};
